...
- **For your assessment you are free and encouraged to make use of JavaScriptFrameworks and tools. 
 
### What about jQuery 

- You may have heard of jQuery. Historically it was a widely used JavaScript library, libraries unlike frameworks do not enforce any coding structure or patterns, you can think of them as extensions of the language. 
...
- When working with any third party library or component you should set up your website project structure as normal 
```
|vue-experiment/
|-index.html
|-css/
...
- Next, in order to include vue into our project we need to download the vendor files. While it's possible to download them from website, we're going to use a package manager.

## The NPM Package Manager
- Using a package manager allows us to easily consume third party packages. Furthermore, it allows you to omit these packages from your git repository. The result is your project is lightweight, portable and will be using up-to-date versions of third party software

![](assets/npm_registry.png)

>> **Figure 1**, The NPM registry ([https://www.npmjs.com/](https://www.npmjs.com/)). 

- While there are several competing front end development package managers, the Node Package Manager (NPM) is by far the most popular
...
- To use the NPM package manager, firstly nodeJS needs to be installed. If you are using cloud9 this has already been done for you. If you are on your own computer, it's a simple case of downloading and click installing the binary from the nodeJs website
...
- Note, while there are lots of files and folders generally the main library will live in the dist folder. E.G. `node_modules/vue/dist/vue.js`

**Ignore the node_modules**

- You should never edit or store the `node_modules` folder in a git repository
- Since our `package.json` contains information about our application's dependancies we don't need to port the `node_modules` folder around
...
- Once installed, Vue can now be included into our project's html pages (Listing 3)

- Notice how we include the javaScript at the bottom of the page. This ensures the HTML is rendered before our javaScript is loaded. Also, notice how our `main.js` file is included after the `vue.js` file. 

![](assets/vue_libary.png)
>> **Figure 4**, The Vue library object. This can be tested by typing `Vue` into your browsers console window
...
- Listing 4 demonstrates how we create a new Vue instance. Notice how we pass in the configure the application using a javaScript object literal containing our app settings	
	
	- `el` is the element that contains our application in this case `#app`
	- `data` is a child object literal that contains data that can easily be rendered in the HTML using the `{{` brackets
...
- The benefits of Vue become even more apparent when we bind to events and work with user inputs

- Consider **Listing 5**, notice on the button we implement the attribute `v-on:click="setMessage`. This will run the corresponding method in `main.js` when the button is clicked
...
- Components can live within other components, hence the idea of Russian dolls
